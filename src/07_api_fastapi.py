...
"""
07api_fastapi.py
- 순수 FastAPI 서버 (LangServe 미사용)
- 엔드포인트:
  1) POST /rag : NIST PDF 기반 RAG (Chroma + OpenAIEmbeddings + LCEL)

필수 설치:
  pip install -U fastapi uvicorn langgraph langchain-core langchain-openai \
...
# ===========================
load_dotenv()  # .env 로드
APP_TITLE = "GCcare NIST RAG API (FastAPI Only)"
APP_VERSION = "1.0.0"
...
def build_rag_chain():
    """
    NIST PDF 기반 RAG 체인 생성.
    - 문서 없을 경우: 빈/기존 벡터스토어만 로드
    """
    # 0) 임베딩 먼저 준비
...
async def rag_endpoint(req: RagRequest):
    """
    질문 문자열을 받아 NIST PDF 기반 RAG로 질의.

    예시 요청(JSON):
    {
      "question": "NIST AI Risk Management Framework의 목적이 뭐야?"
    }
    """
...
print("FastAPI app ready: /health, POST /rag")

"""
실행 예시(터미널):

  python -m uvicorn 07api_fastapi:app --host 127.0.0.1 --port 8000 --reload
